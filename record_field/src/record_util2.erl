...
make_src2([],Acc1)    -> top_and_tail(Acc1);
make_src2([H|T],Acc1) -> NewAcc1=make_get_index(H,[]),
                  make_src2(T,[NewAcc1|Acc1]).


make_get_index([],Acc)->
    Acc;
make_get_index([{RecName,Def}|T],Acc) ->
    make_get_index(T,[expand_fields(RecName,Def,1,[])|Acc])
        .
...
expand_fields(Name,[],N,Acc) -> lists:reverse([mk(Name)|Acc]);
