...
handle_call(world,From,State)->
    ...
    %% 假如，noreply会立即返回，但是如果此时gen_server:reply() 还没有执行完的话
    ...
    %% 注意如果每次call一次，都要启用一个新进程 ，开销有可能依然很大，则可以在第一次call时，注册一个进程
    %% 进行业务逻辑的处理，以后每call一个，则向这个注册的进程 发一个消息，让其处理即可。
    %% 这里一个进程如果不够的话，可能多注册几个进程，组成一个进程池，当收到消息时，随机的发送给这几个进程进行处理
    %% (个人想法，或许不对)
...
handle_call(hi,From,State)->
    ...
    {reply,"hi", State};
handle_call(timeout,From,State)->
    io:format("{reply,timeout}~n",[]),
    io:format("self()=~p,From=~p~n",[self(),From]) ,
    Timeout=1000,
    %% 这里的Timeout
    %% 表示在这次调用以后，在1s之内一定要接到另一个call或cast请求
    %%
...
%% 另外，init/1 的timeout 也可以在此处处理，handle_call等返回值里的timeout也是在这进行处理
%% 但是像gen_server:call 的默认超时5s ，不能。
...
%% handle_call 中返回1s 的timeout
%%E = gen_server:call(P,timeout).
